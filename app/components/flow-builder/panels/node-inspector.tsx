    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3 rounded-lg border border-amber-500/20 bg-amber-500/5 p-3 dark:border-amber-500/30">
        ...
        <label className="flex flex-col gap-1.5">
          ...
          <textarea
            value={data.skill.script ?? ""}
            onChange={(event) => patchSkill({ script: event.target.value })}
            rows={6}
            placeholder="#!/usr/bin/env bash&#10;set -euo pipefail&#10;echo 'Preparing workspace...'"
            className="rounded-lg border border-zinc-200 bg-white px-3 py-2 font-mono text-xs leading-5 dark:border-zinc-800 dark:bg-zinc-950"
          />
          <span className="text-[11px] text-zinc-500">
            Bash script installed as{" "}
            <code className="rounded bg-zinc-500/10 px-1">scripts/run.sh</code> and
            executed before the agent runs.
          </span>
        </label>
      </div>
    ...
    <div className="mb-4 flex items-start justify-between gap-2">
      ...
      {onClose ? (
        <button
          type="button"
          onClick={onClose}
          className="shrink-0 rounded-full px-2.5 py-1 text-xs text-[var(--text-secondary)] transition hover:bg-[var(--surface-muted)]"
      ...
      {onRemove ? (
        <div className="border-t border-[var(--border)] p-4">
          <button
            type="button"
            onClick={onRemove}
            className="w-full rounded-lg border border-red-500/30 bg-red-500/5 px-4 py-2 text-sm font-medium text-red-400 hover:bg-red-500/10"
          >
            Remove node
          </button>
        </div>
