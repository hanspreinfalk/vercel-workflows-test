...
export function hasActiveSkillScript(skill: AgentSkill): boolean {
  return Boolean(skill.name.trim() && skill.script?.trim());
}

export function getSkillScriptPath(skill: AgentSkill): string {
  return `.cursor/skills/${slugifySkillName(skill.name)}/scripts/run.sh`;
}
...
export async function installAgentSkill(sandbox: Sandbox, skill: AgentSkill) {
  ...
  if (hasActiveSkillScript(skill)) {
    files.push({
      path: getSkillScriptPath(skill),
      content: Buffer.from(skill.script!.trim(), "utf-8"),
    });
  }
...
export async function runAgentSkillScript(
  sandbox: Sandbox,
  skill: AgentSkill,
  cwd: string
  ...
  const scriptPath = getSkillScriptPath(skill);
  await sandbox.runCommand({
    cmd: "bash",
    args: ["-lc", `chmod +x ${scriptPath}`],
    cwd,
  });
...
export function skillPromptPrefix(skill: AgentSkill): string {
  ...
  return [
    ...
    hasActiveSkillScript(skill)
      ? `Run the skill script at ${getSkillScriptPath(skill)} first if setup is needed, then execute the task prompt below.`
