...
import { getClaudeSandboxEnv } from "@/lib/claude-agent";
...
export async function runAgentNode(params: {
  ...
  try {
    ...
    const agentPrompt = `${skillPromptPrefix(node.data.skill)}${prompt}`.trim();
    ...
    const cwd = await getNodeSandboxCwd(sandbox, node.data.permissions);
